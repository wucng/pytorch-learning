...
def shutffle_batch(data,batch_size):
    np.random.seed(datetime.now().second)  # reproducible
    np.random.shuffle(data)
    min_after_dequeue = 10
    capacity = min_after_dequeue + 3 * batch_size
    batch_x, batch_y = tf.train.shuffle_batch(
            [data[:,0:-1].flatten(), data[:,-1].flatten()],
...
batch_x, batch_y =shutffle_batch(data,BATCH_SIZE)
    ...
    while not coord.should_stop():
        for epoch in range(1000):
            for step in range(int(np.ceil(len(data)/BATCH_SIZE))):
                if step%100==0:
                    print('Epoch: ', epoch, '| Step: ', step, '| batch x: ',
                          batch_x.eval().flatten(), '| batch y: ', batch_y.eval().flatten())
        else:
            break
