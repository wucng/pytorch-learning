...
# tensorflow代码
有点问题，详细参考[深度学习（08）_RNN-LSTM循环神经网络-03-Tensorflow进阶实现](http://blog.csdn.net/u013082989/article/details/73693392)
...
logits, h_state, init_state = RNN(tf.reshape(_x, [1, 10, 1]), init_state)
total_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=_y))
...
with tf.Session() as sess:
    ...
    for step in range(60):
        ...
        feed_dict = {_x: x, _y: y, init_state: np.zeros([batch_size, hidden_size], np.float32)}
